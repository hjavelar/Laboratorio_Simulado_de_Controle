...
#onready var chart_node = $"Charts/GDCharts"
...
func setPV(value:float):
	PV = clamp(value,0,100)
	Nivel_ProgressBar.value = PV

func setNivel(value:float):
	nivel = clamp(value,0,altura)
	if Conteudo_nivel:
...
func _process(delta):
	...
	if (crt_on && auto_on):  # Ligado no modo automático
		...
		# Limita o PID:
		PID = clamp(P+I+D,0,100)
		
		Vin_Slider.value = PID
...
#	self.nivel = clamp( nivel + vazao*delta/area, 0, altura)
	self.nivel = nivel + vazao*delta/area
